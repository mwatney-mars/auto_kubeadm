  ...
  if [ "$DELETE_OLD_CLUSTER" == "yes" ]; then
    ...
    while [ $i -lt "$QTD_NODES" ]; do
      ...
      if [ "$INSTANCE_CHECK" == "$INSTANCE_NAME-${i}" ]; then
        echo "ERROR: Oh I can't proceed. You have compute instances using the same prefix defined on INSTANCE_NAME ($INSTANCE_NAME). 
...
function enable_autocompletion ()
{
  ...
  gcloud compute ssh $INSTANCE_NAME-1 --zone $GCLOUD_ZONE --command 'echo "source <(kubectl completion bash)" >> ~/.bashrc'
}
